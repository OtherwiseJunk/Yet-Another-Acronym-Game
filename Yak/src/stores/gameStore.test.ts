...
describe("gameStore", () => {
  ...
  it("should send SubmitVoteCommand when submitVote is called", () => {
    store.submitVote("voted-for-user-id");

    expect(mockSend).toHaveBeenCalledWith(new SubmitVoteCommand("voted-for-user-id"));
  });
...
describe("gameStore - not connected", () => {
  ...
  it("should log error when submitVote is called without a connected channel", () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    const store = useGameStore();

    store.submitVote("some-user");
