...
function setImageSources(shouldAnimate: boolean){
    if (props.avatarDecorationUrl) {
        decorationUrl.value = shouldAnimate ? props.avatarDecorationUrl : `${props.avatarDecorationUrl}&passthrough=false`;
    } else {
        decorationUrl.value = undefined;
    }

    if (props.avatarUrl) {
        avatarUrl.value = shouldAnimate ? props.avatarUrl : props.avatarUrl.replace('.gif', '.webp');
    } else {
